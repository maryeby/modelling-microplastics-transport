...
class RelaxingTransportSystem(transport_system.TransportSystem):
	...
	def run_numerics(self, include_history, x_0=0, z_0=0, xdot_0=1, zdot_0=1,
		...
		# run computations
		if include_history:	
			x, z, xdot, zdot, t = self.full_maxey_riley(t_eval, y, order=3)
		else:
...
def alpha(j, n):
	...
	if n == 0:
		...
		if j == 0:
			...
			return 4 / 3 * ((n - 1) ** (3 / 2) - n ** (3 / 2)
					 + (3 / 2) * np.sqrt(n))
		else: # 0 < j < n
			return 4 / 3 * ((j - 1) ** (3 / 2) + (j + 1) ** (3 / 2) \
					 - 2 * j ** (3 / 2))
...
def beta(j, n):
	"""
	Computes the coefficient beta for the specified *j* and *n* as outlined in
	Section 2 of Daitche (2013).
	"""
	if n == 1:
		return alpha(j, 1)
	elif n == 2:
		if j == 0:
			return 12 / 15 * np.sqrt(2)
		elif j == 1:
			return 16 / 15 * np.sqrt(2)
		else:
			return 2 / 15 * np.sqrt(2)
	elif n == 3:
		if j == 0:
			return 4 / 5 * np.sqrt(2)
		elif j == 1:
			return 14 / 5 * np.sqrt(3) - 12 / 5 * np.sqrt(2)
		elif j == 2:
			return -(8 / 5) * np.sqrt(3) + 12 / 5 * np.sqrt(2)
		else:
			return 4 / 5 * np.sqrt(3) - 4 / 5 * np.sqrt(2)
	else:
		if j == 0:
			return 4 / 5 * np.sqrt(2)
		elif j == 1:
			...
			return 176 / 15 - 42 / 5 * np.sqrt(3) + 12 / 5 * np.sqrt(2)
		elif j == n - 1:
			return 8 / 15 * (-2 * n ** (5 / 2) + 3 * (n - 1) ** (5 / 2)
					 - (n - 2) ** (5 / 2)) + 2 / 3 * (4 * n ** (3 / 2)
					 - 3 * (n - 1) ** (3 / 2) + (n - 2) ** (3 / 2))
		elif j == n:
			return 8 / 15 * (n ** (5 / 2) - (n - 1) ** (5 / 2)) + 2 / 3 \
					 * (-3 * n ** (3 / 2) + (n - 1) ** (3 / 2)) + 2 * np.sqrt(n)
		else:
			return 8 / 15 * ((j + 2) ** (5 / 2) - 3 * (j + 1) ** (5 / 2)
					 + 3 * j ** (5 / 2) - (j - 1) ** (5 / 2)) \
					 + 2 / 3 * (-(j + 2) ** (3 / 2) + 3 * (j + 1) ** (3 / 2) \
					 - 3 * j ** (3 / 2) + (j - 1) ** (3 / 2))
...
def gamma(j, n):
	"""
	Computes the coefficient gamma for the specified *j* and *n* as outlined in
	Section 2 of Daitche (2013).
	"""
	if n == 2:
		return beta(j, 2)
		...
		if j == 0:
			return 68 / 105 * np.sqrt(3)
		elif j == 1:
			return 6 / 7 * np.sqrt(3)
		elif j == 2:
			return 12 / 35 * np.sqrt(3)
		else:
			return 16 / 105 * np.sqrt(3)
	elif n == 4:
		if j == 0:
			return 244 / 315 * np.sqrt(2)
		elif j == 1:
			return 1888 / 315 - 976 / 315 * np.sqrt(2)
		elif j == 2:
			return -(656 / 105) + 488 / 105 * np.sqrt(2)
		elif j == 3:
			return 544 / 105 - 976 / 315 * np.sqrt(2)
		else:
			return -(292 / 315) + 244 / 315 * np.sqrt(2)
	elif n == 5:
		if j == 0:
			return 244 / 315 * np.sqrt(2)
		elif j == 1:
			return 362 / 105 * np.sqrt(3) - 976 / 315 * np.sqrt(2)
		elif j == 2:
			return 500 / 63 * np.sqrt(5) - 1448 / 105 * np.sqrt(3) + 488 / 105 \
					   * np.sqrt(2)
		elif j == 3:
			return -(290 / 21) * np.sqrt(5) + 724 / 35 * np.sqrt(3) - 976 / 315\
							   * np.sqrt(2)
		elif j == 4:
			return 220 / 21 * np.sqrt(5) - 1448 / 105 * np.sqrt(3) + 244 / 315 \
					   * np.sqrt(2)
		else:
			return -(164 / 63) * np.sqrt(5) + 362 / 105 * np.sqrt(3)
	elif n == 6:
		if j == 0:
			return 244 / 315 * np.sqrt(2)
		elif j == 1:
			...
			return 5584 / 315 - 1448 / 105 * np.sqrt(3) + 488 / 105 * np.sqrt(2)
		elif j == 3:
			return 344 / 21 * np.sqrt(6) - 22336 / 315 + 724 / 35 * np.sqrt(3) \
					   - 976 / 315 * np.sqrt(2)
		elif j == 4:
			return -(1188 / 35) * np.sqrt(6) + 11168 / 105 - 1448 / 105 \
								* np.sqrt(3) + 244 / 315 * np.sqrt(2)
		elif j == 5:
			return 936 / 35 * np.sqrt(6) - 22336 / 315 + 362 / 105 * np.sqrt(3)
		else:
			return -(754 / 105) * np.sqrt(6) + 5584 / 315
	else:
		if j == 0:
			return 244 / 315 * np.sqrt(2)
		elif j == 1:
			...
			return 5584 / 315 - 1448 / 105 * np.sqrt(3) + 448 / 105 * np.sqrt(2)
		elif j == 3:
			return 1130 / 63 * np.sqrt(5) - 22336 / 315 + 724 / 35 * np.sqrt(3)\
						- 976 / 315 * np.sqrt(2)
		elif j == n - 3:
			return 16 / 105 * (n ** (7 / 2) - 4 * (n - 2) ** (7 / 2)
					  + 6 * (n - 3) ** (7 / 2) - 4 * (n - 4) ** (7 / 2)
					  + (n - 5) ** (7 / 2)) - 8 / 15 * n ** (5 / 2) \
					  + 4 / 9 * n ** (3 / 2) + 8 / 9 * (n - 2) ** (3 / 2) \
					  - 4 / 3 * (n - 3) ** (3 / 2) + 8 / 9 * (n - 4) ** (3 / 2)\
					  - 2 / 9 * (n - 5) ** (3 / 2)
		elif j == n - 2:
			return 16 / 105 * ((n - 4) ** (7 / 2) - 4 * (n - 3) ** (7 / 2)
					  + 6 * (n - 2) ** (7 / 2) - 3 * n ** (7 / 2)) \
					  + 32 / 15 * n ** (5 / 2) - 2 * n ** (3 / 2) \
					  - 4 / 3 * (n - 2) ** (3 / 2) + 8 / 9 * (n - 3) ** (3 / 2)\
					  - 2 / 9 * (n - 4) ** (3 / 2)
		elif j == n - 1:
			return 16 / 105 * (3 * n ** (7 / 2) - 4 * (n - 2) ** (7 / 2)
					  + (n - 3) ** (7 / 2)) - 8 / 3 * n ** (5 / 2) \
					  + 4 * n ** (3 / 2) + 8 / 9 * (n - 2) ** (3 / 2) \
					  - 2 / 9 * (n - 3) ** (3 / 2)
		elif j == n:
			return 16 / 105 * ((n - 2) ** (7 / 2) - n ** (7 / 2)) \
					  + 16 / 15 * n ** (5 / 2) - 22 / 9 * n ** (3 / 2) \
					  - 2 / 9 * (n - 2) ** (3 / 2) + 2 * np.sqrt(n)
		else:
			return 16 / 105 * ((j + 2) ** (7 / 2) + (j - 2) ** (7 / 2)
					  - 4 * (j + 1) ** (7 / 2) - 4 * (j - 1) ** (7 / 2) 
					  + 6 * j ** (7 / 2)) + 2 / 9 * (4 * (j + 1) ** (3 / 2)
					  + 4 * (j - 1) ** (3 / 2) - (j + 2) ** (3 / 2)
					  - (j - 2) ** (3 / 2) - 6 * j ** (3 / 2))
