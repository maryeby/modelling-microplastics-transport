...
from examples.deep_linear_wave.inertial_numerics import AMPLITUDE, WAVELENGTH, \
	 STOKES_HAT, X_0, Z_0, R, NUM_PERIODS, INCLUDE_HISTORY

OUT_FILE = '../data/deep_linear_wave/global_error.csv'
...
def main():
	...
	timesteps = np.logspace(-3, -1, 8, endpoint=False)
	...
	for delta_t in timesteps:
		...
		for order in [1, 2, 3]:
			...
			sol = system.maxey_riley(t, y, include_history=False,
									 order=order)[:2]
			sol = np.array(sol).T
			i += 1
