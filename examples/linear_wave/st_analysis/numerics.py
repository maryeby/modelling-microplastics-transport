...
# particle conditions
X_0 = 0
STOKES_HATS = np.round([67 / 440, 597 / 404, 597 / 202, 67 / 66, 335 / 66], 5)\
				.tolist()
...
RS = np.round([0.66, 101 / 150], 5).tolist()
DELTA_T = 1e-2
NUM_CPUS = None
TIMEOUT = 1200
HIDE_PROGRESS = True
KEYS = ['t', 'x', 'z', 'u_bar', 'w_bar', 'Sthat', 'St', 'Sthat/gamma', 'R',
		'history']
OUT_FILE = '../../data/linear_wave/st_numerics.csv'

def main():
	"""
	Run numerical simulations for inertial particles in a linear wave.

	Simulations are run with and without history effects, with positive and
	negative buoyancy, with various Stokes numbers, and in linear waves of deep
	water. The period-averaged Stokes drift velocity is also computed. Results
