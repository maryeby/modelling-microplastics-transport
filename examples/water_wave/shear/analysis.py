...
from examples.water_wave.shear.numerics import X_0S, RADIUS, ANGLE, Z_NEGATIVE,\
											   ...
											   BETAS, NUM_POINTS, NUM_TASKS
from examples.water_wave.shear.numerics import OUT_FILE as IN_FILE

NUM_CPUS = None
TIMEOUT = 240
OUT_FILE1 = IN_FILE # update input file
OUT_FILE2 = '../../data/water_wave/orbit_shear_difference.csv'
...
def main():
	...
	computed, and results are saved to the `data/water_wave` directory.
	"""
	...
	params = zip(repeat(df), repeated_st, repeated_beta, point_index,
				 repeated_history)
	...
	repeated_st = STOKES_NUMS.tolist() + [ST_TO_SHOW] * len(BETAS)
	repeated_beta = [BETA_TO_SHOW] * len(STOKES_NUMS) + BETAS.tolist()
	results = {'shear_difference': [], 'St': [], 'beta': []}
	params = zip(repeat(df), repeated_st, repeated_beta)
...
def compute_shear(df, stokes_num, beta, i, include_history):
	r"""
	...
	xn, zn = extract_data(['x', 'z'], df, {'history': include_history,
			'x_0': X_0S[i], 'z_0': z_0s[i], 'St': stokes_num, 'beta': beta})
...
def shear_difference(df, stokes_num, beta):
	...
	params = {'St': stokes_num, 'beta': beta, 'history': False}
