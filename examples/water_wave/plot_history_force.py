...
def main():
	...
	water wave, including the history force.
	"""
	...
	x = numerics['x'].where(history).dropna()
	z = numerics['z'].where(history).dropna()
	xdot = numerics['xdot'].where(history).dropna()
	zdot = numerics['zdot'].where(history).dropna()
	t = numerics['t'].where(history).dropna()
	n = t.shape[0] // 4

	fpg_x = numerics['fluid_pressure_gradient_x'].where(history).dropna()
	buoyancy_x = numerics['buoyancy_force_x'].where(history).dropna()
	mass_x = numerics['added_mass_force_x'].where(history).dropna()
	drag_x = numerics['stokes_drag_x'].where(history).dropna()
	history_x = numerics['history_force_x'].where(history).dropna()
	...
	plt.plot(t[:n], history_x[:n], c='cyan', label='history force')
	plt.plot(t[:n], A_x[:n] - G_x[:n], c='k', label='verification')
	...
	# retrieve relevant numerical results
	fpg_z = numerics['fluid_pressure_gradient_z'].where(history).dropna()
	buoyancy_z = numerics['buoyancy_force_z'].where(history).dropna()
	mass_z = numerics['added_mass_force_z'].where(history).dropna()
	drag_z = numerics['stokes_drag_z'].where(history).dropna()
	history_z = numerics['history_force_z'].where(history).dropna()
	...
	plt.plot(t[5:n], history_z[5:n], c='cyan')
	plt.plot(t[5:n], A_z[5:n] - G_z[5:n], c='k')
