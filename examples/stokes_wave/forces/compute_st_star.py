...
from examples.stokes_wave.forces.analysis import OUT_FILE as IN_FILE

TOL = 1e-1
KEYS = ['Sthat*', 'Sthat75', 'St*', 'St75', 'R']
OUT_FILE = '../../data/stokes_wave/forces_st_star.csv'

def main():
	r"""
	Compute $St^*$, where the history force is equal to the Stokes drag.

	The influence of the history force on a particle in a fifth order Stokes
	wave is equal to that of the Stokes drag when the sum of squares of the
	amplitudes of the horizontal component of the history force intersects the
	corresponding sum for the horizontal component of the Stokes drag, as shown
	in subplot *(a)* of the figure produced by `plot_coefficients.py`. This
	intersection value is referred to as $St^*$, and is computed for various
	particle densities. The point at which the history force is 75% of the
	Stokes drag is also computed, $St_{75}$. Results are saved to the
