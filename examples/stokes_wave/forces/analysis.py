...
from examples.linear_wave.forces.numerics import STOKES_HATS, X_0, Z_0, DEPTH, \
	 WAVELENGTH, RS, NUM_PERIODS, DELTA_T, INCLUDE_HISTORY
...
MAXFEV = 10000
OUT_FILE = '../../data/stokes_wave/forces_coeffs.csv'

def main():
	r"""
	Fit curves to numerical forces data and save the associated coefficients.

	For particles of different sizes (Stokes numbers) in a fifth order Stokes
	wave of deep water, a curve is fit to each of the forces over time. The
	coefficents resulting from the curve fittings are saved, and the curves
	are plotted for the particle with Stokes number `ST_TO_SHOW` and density
	ratio `R_TO_SHOW`. Results are saved to the `data/stokes_wave` directory.

	Notes
	-----
	The equation used to fit a curve to the data is,
	$$f(t, A, \delta, \phi, \text{offset}) = A e^{-\delta t} \sin{(t + \phi)} 
	+ \text{offset},$$ and the coefficients saved are the amplitude $A$, decay
	rate $\delta$, phase shift $\phi$, and offset.
