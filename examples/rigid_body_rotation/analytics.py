...
def main():
	...
	varying_timesteps.to_csv(data_path + 'analytics_varying_timesteps.csv',
							 index=False)
	analytics = pd.DataFrame({'t': t, 'x_ana': x, 'z_ana': z})
	analytics.to_csv(data_path + 'analytics.csv', index=False)
	int_times = pd.DataFrame({'int_x': int_x, 'int_z': int_z})
	int_times.to_csv(data_path + 'int_times.csv', index=False)
